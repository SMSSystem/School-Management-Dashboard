/**
 * Firestore Seed Script — client SDK edition (no service account needed)
 *
 * Prerequisites:
...
async function seed() {
  ...
  console.log('🌱  Springfield Academy — Firestore seed starting…\n');

  // ── 1. Institution ────────────────────────────────────────────────────────

  await setDoc(doc(db, 'institutions', iid), {
    name: 'Springfield Academy', institutionId: iid,
    status: 'active', gradingSystem: 'flat',
    location: 'Springfield, West Indies',
    motto: 'Excellence in All Endeavours',
    phone: '+1-868-555-0101', email: 'admin@springfieldacademy.edu',
    address: '1 Academy Drive, Springfield',
    brandColor: '#1a56db', profileComplete: true,
  ...
  const adminDefs = [
    { firstName: 'Eleanor', lastName: 'Hargrove', email: 'ehargrove@springfieldacademy.edu' },
    { firstName: 'Winston', lastName: 'Alleyne',  email: 'walleyne@springfieldacademy.edu'  },
  ];
  const adminUids = [];
  ...
  for (let i = 0; i < SENIOR_TEACHER_NAMES.length; i++) {
    ...
    const email    = `${fn.toLowerCase()}.${ln.toLowerCase()}@springfieldacademy.edu`;
    const name     = `${fn} ${ln}`;
  ...
  for (let i = 0; i < STUDENT_NAMES.length; i++) {
    ...
    const email     = `${fn.toLowerCase()}.${ln.toLowerCase()}.student@springfieldacademy.edu`;
    const name      = `${fn} ${ln}`;
  ...
  for (let i = 0; i < PARENT_NAMES.length; i++) {
    ...
    const email    = `${fn.toLowerCase()}.${ln.toLowerCase()}.parent@springfieldacademy.edu`;
    const name     = `${fn} ${ln}`;
    const uid      = await createAuthUser(email, name);
    await setDoc(doc(db, 'users', uid), {
      uid, firstName: fn, lastName: ln, name, email,
  ...
  console.log('───────────────────────────────────────────────');
  console.log('  Admin 1        : ehargrove@springfieldacademy.edu');
  console.log('  Admin 2        : walleyne@springfieldacademy.edu');
  console.log('  Senior Teacher : margaret.thornton@springfieldacademy.edu');
  console.log('  Regular Teacher: alice.fletcher@springfieldacademy.edu');
  console.log('  Student        : aiden.clarke.student@springfieldacademy.edu');
  console.log('  Parent         : michael.clarke.parent@springfieldacademy.edu');
  console.log('═══════════════════════════════════════════════\n');
}
