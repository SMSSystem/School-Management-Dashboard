...
const TYPES: DisciplinaryActionType[] = [
  "merit",
  "demerit",
  "detention",
  "suspension",
];
const NEEDS_DATE_RANGE = new Set<DisciplinaryActionType>([
  "detention",
  "suspension",
]);
...
const DisciplinaryActionForm = ({
  ...
  const [studentQuery, setStudentQuery] = useState(
    studentLocked ? String(data?.studentName ?? "") : "",
  );
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    try {
      ...
      if (type === "create") {
        ...
        if (typeof id !== "string") {
          console.log(
            "DisciplinaryActionForm update: no string ID (mock mode)",
            formData,
          );
          return;
        }
        ...
        await updateDoc(
          institutionDoc(institutionId, "disciplinaryActions", id),
          {
            type: formData.type,
            reason: formData.reason,
            date: formData.date,
            endDate:
              needsRange && formData.endDate ? formData.endDate : deleteField(),
