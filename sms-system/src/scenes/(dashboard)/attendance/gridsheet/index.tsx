...
import { db, ClassDocument, GeneralAttendanceDocument, TermDocument } from '@/lib/firebase';
              ...
              return sessions.map((se) => (
                <th
                  ...
                  className={`w-8 px-0.5 py-1 text-center font-medium text-gray-600 dark:text-gray-400 last:border-r border-gray-200 dark:border-gray-700 ${
                    se.session === 'PM' ? 'border-r border-gray-200 dark:border-gray-700' : ''
                  } ${band}`}
                  title={`${se.date} ${se.session}`}
                >
                  {se.dayIndex}{se.session === 'AM' ? 'A' : 'P'}
                </th>
              ));
            })}
          </tr>
...
export default function AttendanceGridsheetPage() {
  ...
  const { assignedClassId, assignedClassName, loading: profileLoading } =
  ...
  // ── Guard conditions ──
  if (USE_MOCK) {
    return <InfoState message="Attendance Summary Register is not available in demo mode." />;
  }
      ...
      {gridData && gridData.studentRows.length > 0 && (
        <>
          ...
          <section>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
              Number of students present per session. Column labels: day index within
              month + A (AM) or P (PM). Students with no gender recorded are included
