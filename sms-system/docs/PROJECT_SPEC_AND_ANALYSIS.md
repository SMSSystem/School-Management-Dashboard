...
- **Own reports:** generated on demand from grades + teacher feedback for the selected term.
...
| A-3 | Report generation | A-2, D-5 | On-demand join of `results` + `feedback_comments` for a given `studentId + termId`; viewable by student, parent, institution admin |
