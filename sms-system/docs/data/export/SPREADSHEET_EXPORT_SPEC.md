...
| Results export access         | **Admin/teacher roles only, regardless of page-view access**                                                                                                                                                    | The Results list page's live query is not scoped by student/parent identity — every `student`/`parent` who opens `/dashboard/list/results` today sees every result in the institution (see §6, §9). Exposing that as a downloadable file would make an existing narrow, easy-to-miss gap into a one-click bulk export. Restricting the **export button** to `institution_admin` / `super_admin` / `senior_teacher` / `regular_teacher` avoids compounding the gap without requiring a fix to the underlying page query as a prerequisite for this feature. The underlying query gap itself is out of scope for this spec (see §15) and should be tracked separately. |
...
- The page subscribes via `onSnapshot` to the entire `results` collection filtered only by `institutionId`, with a client-side filter to exclude gradebook-originated rows (`gradebookColumnId` present).
- **No class/term/subject filter UI exists on this page today** — it lists every manually-entered result in the institution at once.
- Visible to all 6 roles per `Menu.tsx`/no route guard in `App.tsx`; row-level create/edit/delete actions are role-gated, but the _view_ itself is not scoped by student/parent identity (see §2's decision log entry and §9).

### 6.2 Export design

Per §2's decision, export is **not** simply "serialize whatever's currently in the unfiltered `onSnapshot` state" — two changes are needed on top of what the page has today:

1. **A filter UI is added as a prerequisite** — class, subject, and term `<select>`s (all optional/"All"), placed above the existing table, mirroring the filter-selector pattern already used on Gradebook/Gridsheet/Report Cards. This is new UI, not present on the page today.
2. **Export runs a fresh, filtered `getDocs` query** rather than reusing the live `onSnapshot` state, using whatever subset of `institutionId` / `classId` / `subjectId` / `termId` the admin/teacher selected. This query is served by the existing composite index (`results`: `institutionId ASC, classId ASC, subjectId ASC, termId ASC` — already deployed per `firestore.indexes.json`); a partial subset of these equality filters (e.g. `institutionId` + `termId` only, with class/subject left as "All") is also servable without any _additional_ index, since Firestore serves multi-field equality-only queries via automatic single-field indexes regardless of which subset of fields is filtered — composite indexes are only mandatory once a range/inequality filter or a multi-field `orderBy` enters the query, neither of which applies here.

**Export button:** rendered only for `institution_admin`, `super_admin`, `senior_teacher`, `regular_teacher` — never for `student`/`parent`, regardless of the underlying page-view gap (§2, §9).

**Columns** (`ExportColumn<ResultDocument & {id: string}>[]`):

| Header     | Source field                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Student    | `studentName` (already present on written docs though not in the declared `ResultDocument` type — see §6.3)                                                                                                                                                                                                                                                                                                                                                                 |
| Class      | `className` (same note)                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Teacher    | `teacherName` (see §6.3) / falls back to a teacher-id lookup if absent                                                                                                                                                                                                                                                                                                                                                                                                      |
| Source     | derived: `"Gradebook"` if `gradebookColumnId` present, else `"Manual entry"` — included for completeness even though this page filters gradebook-originated rows out of its own on-screen table, since the _export_ query (being fresh, not reusing the filtered on-screen state) may reasonably choose to include both; **default is manual-entry-only, matching the page's current scope**, with an "Include gradebook-originated results" checkbox as an explicit opt-in |

### 6.3 Data model note

`ResultDocument`'s declared TypeScript type (`firebase.ts` lines 120–134) is narrower than what's actually written to Firestore — Gradebook's `performSave` writes additional fields (`studentName`, `className`, `gradebookColumnId`, `columnWeight`, `source`, `teacherName`, `createdAt`) that aren't part of the declared type. The export column mapping above accounts for this by treating those fields as optional/`unknown`-typed reads with fallbacks, not by assuming the declared type is complete. Widening `ResultDocument` itself to match reality is a reasonable small cleanup but is not required for this feature to work correctly.
...
| Results      | `institution_admin`, `super_admin`, `senior_teacher`, `regular_teacher` | **Narrower than page-view access** — `student`/`parent` can view the page (and, today, see all institution results while doing so — a pre-existing gap) but will not see an export button. See §2 decision log and §15. |
...
5. Results: add the class/subject/term filter UI (prerequisite, §6.2), the filtered `getDocs` export query, column mapping (§6.2), and `ExportMenu`, gated to admin/teacher roles.
...
- Fixing the Results page's underlying **student/parent visibility gap** (§6.1, §9) — this spec routes around it for the export button specifically but does not fix the page's own query scoping; tracked as a separate, pre-existing issue.
...
- **Results page filter UI is net-new** — this spec assumes class/subject/term `<select>`s get added to `list/results/index.tsx` as part of implementing export; if that UI is considered too large an addition for this feature, the alternative is exporting the page's already-loaded (currently unfiltered, institution-wide) state as-is, which is simpler but reintroduces the "export everything at once" concern this spec's decision log (§2) was written to avoid. Recommend keeping the filter UI.
...
- **`ResultDocument`'s declared type is narrower than what's actually written** (§6.3) — the export's column mapping works around this with optional/fallback reads; widening the type is optional cleanup, not a blocker.
- **This spec does not fix the Results page's student/parent over-visibility gap** — it only prevents the export feature from making it worse. If that gap gets fixed independently before this feature ships, §6/§9's admin/teacher-only export restriction can be revisited (e.g. loosened to match corrected page access) but is not required to change.
