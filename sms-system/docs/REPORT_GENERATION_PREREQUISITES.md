...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Generate (write to `reports`) | ❌ | ❌ | ✅ dept scope | ✅ class scope | ✅ own only | ❌ |
...
|---|---|
| `student` | Their own report only (`studentId == request.auth.uid`) |
| `regular_teacher` | Any student in a class they are the assigned class teacher for (`isClassTeacherFor`) |
| `senior_teacher` | Any student in any class within their department (`isSeniorTeacherFor`) |

#### 2.1.1 Flag — Admin Re-generation Removed

The original spec (§1.8) said:

> Institution Admin can view and re-generate any report across the school.

This has been removed. If an admin needs to update a report after a grade correction, they have no path to force a refresh without going through a teacher. This is an intentional product decision, but worth revisiting before A-3 is built if this turns out to be a UX problem in practice.
...
reports/{docId}             // auto-generated ID
  ...
  feedback         array    // snapshot of feedback_comments for this student+term
...
match /reports/{docId} {
  ...
  // Students generate their own report only.
  // Teachers generate for students within their class or department scope.
  // Admins do not generate (read-only per §2.1 decision).
  allow create: if writingToMyInstitution()
    && (resource == null  // only on creates, not updates
...
> **Note on `classId` in reports:** For `isClassTeacherFor` to resolve correctly, the `reports` document must store a `classId` at write time. Because a student may be enrolled in multiple classes across a term, a report may span multiple classes. Consider whether a report should be scoped to a single class or to the full term across all classes. If the latter, `classId` cannot be a single field — the teacher write rule will need a different scope mechanism. This is an additional design point to resolve before A-3 is built.
...
| **N-6** | Should a `reports` document be scoped to a single class or to all classes for a student in a term? | **Unresolved.** Affects the `classId` field on `reports` documents and the teacher write rule design (see §6.2 note). Must be decided before A-3 Firestore rules are finalised. |
