...
4. **Merge `data-structure-overhaul` into `main`.** This starts Vercel's build/deploy of every phase's nested-path-reading code at once (§11's deploy-ordering note) — it does not mean that code is live yet.
5. **Confirm the Vercel deploy of merged `main` is actually live** (check the live site or the Vercel dashboard's deployment status — do not just assume the merge itself was enough) — **then** apply both of §10.9's pending diffs to `firestore.rules` in one pass and deploy once via `npm run firebase:deploy:rules`, after the same mandatory `git diff 6df4d98~1 -- firestore.rules | grep '^-'` check used before every other deploy this overhaul. **This ordering is load-bearing, not incidental:** diff #1 specifically requires `main`'s new code (which stops writing `departmentId` to the `teachers` mirror) to already be live — applying it merely "alongside" the merge, before Vercel's build finishes, reopens the exact class of bug that caused both incidents in §10.8. Diff #2 has no such dependency (only needs step 2's data copy, already satisfied) and could technically deploy earlier, but is bundled here anyway to keep this to one rules deploy instead of two.
6. **End the maintenance window**; smoke-test the golden path per role across the whole app (§11 step 11) — the same per-role spot-check pattern used throughout this session, now covering every migrated collection at once rather than one phase's worth.
7. **Only after confirming the app is correctly reading/writing the new paths in normal use**, run the script's deletion pass (`--delete-source`), collection by collection. This is a deliberate, separate step — there's no fixed timeline for when it must happen; leaving the verified-migrated flat originals in place for a few extra days as a safety buffer costs nothing but a small amount of the 1 GiB storage quota (§14.2).
...
**Confirmed pilot-scale check-in:** as of this writing, pilot deployment is 1–2 institutions with light data — comfortably within §14.7's per-institution estimates for a single combined copy pass (cutover step 2). Re-confirm this before actually running the cutover if pilot institution count or data volume has grown materially since — §14.7 explicitly flags "migrate every phase for every pilot institution on the same calendar day" (which this combined-cutover plan now does by design) as the one scenario that could meaningfully approach the 20,000-write daily quota; if it no longer holds, split cutover step 2 across two calendar days by institution rather than running it against a growing pilot base in one pass.

### 10.5 Safety properties of this ordering

- **Nothing is destroyed until cutover step 7 explicitly runs.** If cutover step 4 or 5 reveals a problem, the merge can be reverted (or a fix rolled forward) and the app keeps working against the untouched flat originals — the copy made in cutover step 2 is simply abandoned or rerun, at no data-loss risk. If step 5 specifically fails (Vercel deploy doesn't go live cleanly), do not apply §10.9's diffs — the flat rule blocks stay exactly as `main` needs them either way, so an unconfirmed or reverted merge leaves production untouched.
...
- **The maintenance window needs to cover cutover steps 2–6**, not just 2–3 — once the copy finishes and is spot-checked, the merge (step 4) can happen right after, but the window stays open through step 5's Vercel-live confirmation and diff deploy, closing only once the app is confirmed working in step 6. This is wider than earlier drafts of this section assumed (see §10.8's incident writeup for why collapsing "merge" and "diffs are safe to apply" into one moment was the mistake).
...
### 10.8 Incident: `firestore.rules` deploys are whole-file, not per-block — broke production twice

**What happened.** The plan from step 6 onward assumed a rules deploy could be scoped to "just this phase's safe part" — deploy the additive new blocks, leave a phase's destructive change (one that modifies an *existing* block `main` still depends on) sitting undeployed in the same file until the cutover. That assumption is wrong: `firebase deploy --only firestore:rules` uploads the entire current `firestore.rules` file, every time, with no mechanism to deploy a subset of match blocks. Two destructive changes were sitting in the working file when unrelated, believed-safe deploys went out, and both went live by accident:

1. **Step 4's `teachers`/`students` block removal + `isSeniorTeacherFor()` rewrite** (§11 step 4) was written to the branch and explicitly marked "hold until `main` merges" — but it stayed in `firestore.rules` the whole time. The **step 5 deploy** (believed purely additive — new `houses`/`departments`/etc. blocks) carried it live anyway, because it was the same file. From that point, `main`'s still-live `AdminCreateUserForm.tsx`/`TeacherForm.tsx` (which only write `departmentId` to the `teachers` mirror) had no rule permitting writes to `teachers`/`students` at all, and `isSeniorTeacherFor()` was reading `users.departmentId`, which the backfill script had never populated (it can't — `main` doesn't have the code that writes it yet). Teacher/student creation and edits, and senior-teacher department-scoped permissions, were broken in production for however long elapsed between the step 5 deploy and this fix.
2. **Step 6's `results`/`feedback_comments`/`subjectAttendance` `get()` repointing** (their internal ownership check, repointed at the not-yet-populated nested `subjects` path) was written to the branch with the same "hold for cutover" intent. The **step 7 deploy** (believed purely additive — new `exams`/`assignments`/etc. blocks) carried it live too. `regular_teacher` create/update on results and feedback comments, and read/create/update on subject attendance, were broken for the gap between that deploy and this fix (admins and senior teachers were unaffected — their branches don't hit that `get()` call).

**Both fixed the same way**, immediately on discovery: revert the destructive lines back to what `main` actually depends on (flat `subjects` reference; restored `teachers`/`students` blocks and mirror-reading `isSeniorTeacherFor()`), redeploy immediately, then verify via `git diff <last-known-safe-commit> -- firestore.rules | grep '^-'` returning empty — i.e., the working file is a strict superset of the last commit known not to break `main`, zero deletions or modifications to anything `main` depends on.

**Root cause, precisely:** not "additive vs. destructive" reasoning per se (that reasoning was correct for judging any single phase's *own* new blocks) — the mistake was treating each phase's rules change as independently deployable when the deploy mechanism itself has no such granularity. A phase's blocks being additive says nothing about whatever *other*, not-yet-cutover-ready content might also be sitting in the same file at deploy time.

**Process fix, binding from here on (§11 step 8 onward):** `firestore.rules` in the working tree must **only ever contain content safe to deploy immediately**, in full, at any moment. A destructive change — anything that modifies or removes an existing block `main`'s live code still depends on — is never written into `firestore.rules` directly. Instead:

1. Write the change as an explicit before/after diff in §10.9 below ("Pending cutover-only rules changes"), not in `firestore.rules` itself.
2. Leave the live block in `firestore.rules` exactly as `main` needs it, until the cutover.
3. At the actual cutover, apply every diff in §10.9 to `firestore.rules` in one pass at **§10.4 cutover step 5** — **not** "alongside" cutover step 4 (the merge itself). **Correction found during step-11 planning:** the original wording here said "right before/alongside the merge," which is exactly the kind of gap that caused both incidents above — merging doesn't make `main`'s new code live instantly, Vercel still has to build and deploy it. Diff #1 specifically depends on that new code already being live (see its own prerequisite below); deploying it merely alongside the merge reopens the same failure mode. §10.4 cutover step 5 is the explicit checkpoint: confirm Vercel's deploy of merged `main` is actually live, *then* apply both diffs and deploy once.
4. Before *any* rules deploy between now and the cutover, run `git diff 6df4d98~1 -- firestore.rules | grep '^-'` (the commit immediately before step 4 — the last state confirmed to not break `main`) and confirm it's empty. A non-empty result means something destructive has leaked into the working file and must be moved to §10.9 before deploying.

### 10.9 Pending cutover-only rules changes
...
+                   /databases/$(database)/documents/institutions/$(request.resource.data.institutionId)/subjects/$(request.resource.data.subjectId)
```

(and the `resource.data.institutionId` / plain path-bound `institutionId` variants — 6 call sites total across the 3 blocks; exact text is in git history at commit `eebe1f9`, the step 6 commit, before this incident's revert.) **Prerequisite before applying:** `subjects` data must already exist at the nested path (cutover step 2 must have run first).
...
**Deploy-ordering constraint discovered at step 4, refined for the remaining phases, then corrected hard after a real incident at steps 5 and 7 (§10.8):** `main`, not this branch, is what's deployed to production (Vercel, auto-deploy on merge) and what pilot users hit. Step 4's rules change was blocked from deploying because it was *destructive* — it removed the `teachers`/`students` rule blocks that `main`'s still-live code depends on, and repointed `isSeniorTeacherFor()` away from data `main` still writes. The plan through step 7 tried to have it both ways — deploy a phase's additive new blocks now, leave its destructive part sitting undeployed in the same file for the cutover — and that broke production twice, because **`firestore.rules` deploys are whole-file**: there is no way to deploy part of it. Both step 4's and step 6's destructive changes went live by accident via later, believed-safe-and-unrelated deploys (step 5's and step 7's). Full incident writeup and the fix in §10.8.

**Binding process from here on:** `firestore.rules` in the working tree only ever contains content safe to deploy immediately, in full. A destructive change (modifies or removes a block `main`'s live code still depends on) is never written into the file — it's tracked as an explicit diff in §10.9 and applied only at the cutover, all at once, right before that final deploy. A brand-new `institutions/{id}/{collection}` match block that nothing else references is still fine to add to the working file and deploy as soon as it's written (confirmed safe for phases 5 and 7 — grepped the whole file for `get()`/`exists()` calls into each phase's collections, found none from any other block). But **before recommending any deploy, run `git diff 6df4d98~1 -- firestore.rules | grep '^-'`** (the commit right before step 4, the last state confirmed not to break `main`) and confirm it's empty — not just reason qualitatively about "this phase's diff is additive." What's always blocked regardless of any of this: merging any phase's **app code** into `main` (nothing at the nested paths for it to read until the cutover copies data). Confirmed with the user: phases 5–12 finish on this branch, then merge into `main` in a single event at the end — not a per-phase hotfix pattern.
...
**Postscript — this held-back change went live by accident anyway (§10.8).** "Not yet deployed" wasn't actually true in practice: the destructive edit stayed in `firestore.rules` and the step 5 deploy (believed unrelated and purely additive) pushed it live, breaking teacher/student creation and senior-teacher permissions in production for real. Reverted and redeployed on discovery — `teachers`/`students` blocks and the mirror-reading `isSeniorTeacherFor()` are restored in the working file exactly as `main` needs them. The actual change described above is now tracked as an explicit diff in §10.9, to be applied only at the cutover, not left sitting in `firestore.rules`.
5. **Nest the low-traffic, no-cross-collection-dependency group — build done, rules deployed.** `houses`, `departments`, `events`, `announcements`, `nonSchoolDays`, `academicYears` — simplest Pattern A collections (§5.2), nothing else reads from them via `get()`/`exists()` in rules. Added `inMyInstitution()`/`institutionFieldMatchesPath()` (§5.1) and the six `institutions/{institutionId}/{collection}` rule blocks to `firestore.rules`, alongside (not replacing) the still-authoritative flat blocks, each now commented "Still authoritative — ... same cutover" to flag the pending removal. Permission levels copied exactly from each flat block (`houses`/`nonSchoolDays`/`academicYears` stay `isAdmin()`-only; `departments`/`events`/`announcements` stay `isAdminOrAbove()`) — not the spec's generic Pattern A example, which would have loosened `houses`/`nonSchoolDays`/`academicYears` to admin-or-above. **Deployed via `npm run firebase:deploy:rules`** — compiled and released cleanly, no unused-function warnings this time (both new helpers have real callers, unlike step 3's `isClassTeacherFor`). App code: every call site across 13 files (`HouseForm`, `StudentForm`, `DepartmentForm`, `TeacherForm`, `AdminCreateUserForm`, `FormModal` (added a `NESTED_TABLES` set for its generic delete path), the `houses`/`departments`/`events`/`announcements` list + `houses/[id]`/`students/[id]` detail pages, `reports/builder`, `useInstitutionAcademicCalendar`, `academic-calendar`, `admin/rebuild-attendance-summaries`, `generateReportCard.ts`, `useTourSteps.ts`) now reads/writes via `institutionCollection()`/`institutionDoc()` (§9.1) for these 6 collections only — every other collection in the same files (`users`, `classes`, `terms`, `subjects`, `generalAttendance`, etc.) deliberately left untouched, still flat, for its own later phase. Committed to the branch, not merged — `tsc -b`, `eslint`, and `npm run build` all clean. **Not manually browser-tested**: the dev server hits the same live `school-sms-v1` project, and no data exists yet at the nested paths (§10.4's copy step is deferred to the cutover) — a live run would just show empty lists either way, which isn't a meaningful check of query correctness. Verification for this phase rests on type-checking plus code review, not UI testing; full UI verification happens at the cutover's post-merge smoke test (§11 step 11).
6. **Nest the curriculum backbone — build done.** `subjects`, `classes`, `terms` — still Pattern A. **Second correction, found while implementing:** §5.3's audit table (and this step's own original wording) was right about the *fact* — some still-flat rule reaches into `subjects` via `get()` — but wrong about *which* collection: `generalAttendance` and `timetable_slots` were named, but neither actually references `subjects`; `generalAttendance`'s `get()` calls read `users` (unchanged) and `timetable_slots`'s reads `users.canGenerateSchedule` (unchanged). The real 6 call sites (confirmed via `grep`) live in `results` (2, phase 7), `feedback_comments` (2, phase 7), and `subjectAttendance` (2, phase 8) — none of which are nested by this step either; only their *internal reference* to `subjects` changes, because `subjects` itself is moving, not because those collections are. `classes`/`terms` have zero cross-references (confirmed via the same grep — nothing in the live rules file does `get()` into either). Added `institutions/{institutionId}/{subjects,classes,terms}` (additive, `isAdminOrAbove()` uniformly, matching all three flat blocks exactly) plus repointed all 6 `get(.../subjects/$(id))` calls in `results`/`feedback_comments`/`subjectAttendance` at `institutions/$(institutionId)/subjects/$(id)`, using `resource.data.institutionId`/`request.resource.data.institutionId` (there's no `institutionId` path variable in those still-flat blocks to reference directly). Intended to stay undeployed until the cutover. (`isClassTeacherFor` no longer needs updating here — removed in step 3, §5.3.) **Postscript — this went live by accident anyway, broke production, and was reverted (§10.8).** The repointed `get()` calls sat in `firestore.rules` "held for the cutover," but the step 7 deploy (believed unrelated and purely additive) pushed them live too — `firestore.rules` deploys are whole-file, there's no partial deploy. `regular_teacher` create/update on results/feedback comments and read/create/update on subject attendance were broken in production until this was caught and reverted back to the flat `subjects` reference. The repointing is now tracked as an explicit diff in §10.9 instead, applied only at the cutover. App code: same 38-file sweep pattern as step 5, delegated to a subagent given the volume (~70 call sites) and verified afterward (`tsc -b`/`eslint`/`build` clean, diff spot-checked, full flat-call-site grep sweep confirms zero stragglers) — every other collection in each touched file left alone. Data copy: same deferral as step 5. **Also found, out of scope for this step:** `src/lib/generateReport.ts` (called from the orphaned, unrouted `src/scenes/(dashboard)/reports/index.tsx` — not the report-*card*-builder) still writes to the flat `reports` collection, which step 3 deleted the rules for entirely; harmless only because nothing imports that scene from `App.tsx` or any nav menu, so the write path is unreachable. Candidate for deletion during step 12 cleanup, not fixed now since it's dead code either way.
7. **Nest the teaching-data group — build done.** `exams`, `assignments`, `results`, `feedback_comments`, `lessons`, `timetable_slots`, `subjectEnrollments`, `gradebooks`+`columns` — Pattern B/D, highest call-site count *and* highest document count of any phase. **Cross-reference assessment (per step 6's method): none of these 8 needed it.** Grepped the whole rules file for `get()`/`exists()` calls into any of the 8 — zero hits from any *other* block, so nesting them adds new blocks only, touching nothing existing; unlike step 6, this phase's rules are purely additive end to end. (Their own outbound reference — results/feedback_comments into `subjects` — was already handled in step 6.) Added all 8 `institutions/{institutionId}/{collection}` blocks (`results`/`feedback_comments` reusing the path-bound `institutionId` variable for their nested `subjects` lookup, cleaner than step 6's flat-block `resource.data.institutionId` workaround; `gradebooks`/`columns` per §5.2 Pattern D verbatim). **Deployed** — this phase's own blocks were genuinely additive and are correctly live. **But this deploy is also the one that carried step 6's held-back destructive change live by accident (§10.8)** — the postscript on step 6 above covers what broke and how it was fixed. §11's preamble and §10.8/§10.9 now carry the corrected, binding process for every deploy from here on: nothing destructive goes into `firestore.rules` at all until the cutover. App code: same 8-collection sweep across ~24 files (delegated to a subagent, verified after — `tsc -b`/`eslint`/`build` clean, same baseline, independent grep sweep confirms zero flat stragglers). `list/gradebook/index.tsx` was the single densest file in the whole overhaul so far (7 call sites across `results`, `feedback_comments`, and `gradebooks`+`columns` in one file) — spot-checked by hand in full. `LessonForm.tsx` has no real Firestore calls (`console.log` stub) — nothing to convert there. Also fixed a now-doubly-stale JSDoc example in `src/lib/utils.ts` (pointed at `results`, which just moved) to describe the pattern generically instead of naming a specific collection, so it stops going stale every phase.
8. **Nest the attendance group — build done.** `generalAttendance`, `subjectAttendance`, `attendanceSummaries` — most complex role logic (§5.2 closing note); this is also likely the largest single migration pass by document count (§14.7) — budget the combined cutover's maintenance window accordingly. **Cross-reference assessment (per step 6/7's method): zero external references.** Grepped the whole rules file for `get()`/`exists()` calls into any of these 3 collection names from any *other* block — none found, so all three new blocks are purely additive, touching nothing existing. `subjectAttendance`'s own internal `regular_teacher` ownership check (its outbound reference into `subjects`) is written in the *new* nested block pointing directly at `institutions/$(institutionId)/subjects/$(...)` — safe because that data has existed since step 6's deploy, and because this is a brand-new block, not a modification of the still-live flat `subjectAttendance` block (which stays exactly as `main` needs it, still reading flat `subjects`, per §10.9's pending diff #2 — unchanged by this step). Role logic (student/parent/senior-teacher/admin branches, the `attendanceSummaries` `resource == null` probe guard) carried over unchanged in substance; only `sameInstitution()`/`writingToMyInstitution()` became `inMyInstitution(institutionId)`/`institutionFieldMatchesPath(institutionId)` throughout, per §5.2's closing note — plus one small improvement over the flat version: `attendanceSummaries`' null-resource read branch can now check `inMyInstitution(institutionId)` too (the flat version couldn't, since there's no `resource.data` to read on a non-existent doc; the nested version has the institution from the path segment regardless). Confirmed deployable now: the mandatory `git diff 6df4d98~1 -- firestore.rules | grep '^-'` check returns empty. App code: 12 files (`attendanceSummaryUtils.ts`, `AttendanceChart.tsx`, `AttendanceScopeModal.tsx`, `attendance/my`, `attendance/gridsheet`, `attendance/general`, `attendance/child`, `attendance/subject`, `senior-teacher`, `admin/rebuild-attendance-summaries`, `admin/index.tsx`, `generateReportCard.ts`) — delegated to a subagent given the volume, verified after (`tsc -b`/`eslint`/`build` clean, same baseline, independent grep sweep confirms zero flat stragglers; hand spot-checked the two create-or-update + live-listener files (`attendance/general`, `attendance/subject`) and the cross-collection `rebuildSummariesForClass` rebuild pipeline in `attendanceSummaryUtils.ts` in full). `attendanceSummaries` documents keep their deterministic `${studentId}_${termId}` ID unchanged, now via `institutionDoc(institutionId, 'attendanceSummaries', ...)`. Not manually browser-tested, same reasoning as step 5 (no data exists yet at the nested paths until the cutover's copy step) — full UI verification deferred to the post-merge smoke test (§11 step 11).
9. **Nest the report-card and disciplinary group — build done.** `studentActivities`, `studentResponsibilities`, `reportCardComments`, `reportCards`, `disciplinaryActions` — Pattern C. **Cross-reference assessment (per step 6/7/8's method): zero external references, and none of these 5 has an outbound `get()`/`exists()` of its own either** (unlike `subjectAttendance` in step 8) — grepped the whole rules file for calls into any of the 5 collection names from any other block, found none; all five new blocks are purely additive with nothing to repoint. `reportCardComments` keeps its narrower read rule (`isAdminOrAbove()` only, no teacher/student/parent branch — comments aren't exposed below admin) rather than the generic Pattern C shape; the other four match Pattern C as given. Confirmed deployable now: the mandatory `git diff 6df4d98~1 -- firestore.rules | grep '^-'` check returns empty. App code: 8 files, delegated to a subagent given the volume and verified after (`tsc -b`/`eslint`/`build` clean, same baseline, independent grep sweep confirms zero flat stragglers). `generateReportCard.ts` (§9.3) was the densest single file — 7 call sites across all 5 collections in one function, using the already-required `opts.institutionId` parameter throughout; hand spot-checked in full, including the upsert's `addDoc`/`updateDoc` branch. `list/students/[id]/index.tsx` had three query effects (`studentActivities`/`studentResponsibilities`/`disciplinaryActions`) that carried no `institutionId` filter at all pre-migration (only `studentId`/`termId`) — added an `institutionId` guard to each, matching the guard style already used by that file's `reportCardComments` effect, since the nested path now requires it where the old flat query didn't strictly need it. `FormModal.tsx`'s `NESTED_TABLES` gained `disciplinary_action` (its `collectionNameFor` override to `disciplinaryActions` was already correct, untouched). `report-cards/index.tsx`'s batch-generate `writeBatch` needed no direct change — it operates on `d.ref` from an already-converted query snapshot, so the nested path flows through automatically. Data copy: same deferral as step 5.
10. **`collectionGroup` rules — build done.** Added the `super_admin`-only `match /{path=**}/{collection}/{docId} { allow read: if isSuperAdmin(); }` rule (§5.5, following the existing `audit_log`/`activity_log` precedent exactly) for all 25 collections nested in phases 5–9 (§4.1's count), grouped by step in `firestore.rules` for readability. **Overlap with still-authoritative flat blocks, confirmed harmless:** `{path=**}` matches a collection at any depth including depth 1, so e.g. `match /{path=**}/results/{resultId}` also matches the still-live flat `/results/{resultId}` documents, in addition to the new nested ones — Firestore rules are OR'd across matching blocks, so this only *adds* a `super_admin` read path onto the flat collection, it doesn't restrict or replace the flat block's own rule (which already lets `super_admin` through via its `sameInstitution()`/`isSuperAdmin()` bypass anyway) — redundant, not destructive. No indexes added (§6.2 — deferred until a consumer exists). No data movement in this step. Confirmed deployable now: the mandatory `git diff 6df4d98~1 -- firestore.rules | grep '^-'` check returns empty; brace-balance checked (270 open / 270 close). Purely rules-only — no app code changes in this step (§3.6/§5.5's "no consuming UI yet" framing, §12).
...
12. **Documentation — done.** `firebase-rules.md`'s header already carries the CLI-is-authoritative convention change (§7.2) from earlier in this branch's history (the `af2de0c`/`bcd53e3` commits, before this session's step 5 work began) — confirmed still accurate, nothing further needed. No separate `docs/firebase/firestore_indexes` file exists in this repo; `firebase-rules.md`'s "Composite index notes" section already fills that role — it holds only the non-obvious "why" for indexes worth explaining and explicitly defers to `firestore.indexes.json` as the actual current inventory (see the postscript below for an addition to this section, found afterward). **Deleted the orphaned dead code flagged in step 6's writeup:** `src/scenes/(dashboard)/reports/index.tsx` and `src/lib/generateReport.ts` — confirmed via grep immediately before deleting that nothing else in `src/` imports either (the sibling `reports/builder/` directory, which *is* live, was left untouched), and confirmed via `tsc -b` afterward that removing them introduced zero new errors. `eslint`/`build` re-verified clean against the same known baseline.

**Postscript — a real gap found during step-11 cutover planning, not part of the original 12 steps: §6.1's composite-index work was never assigned to a step.** §6.1 documents that `generalAttendance` and `subjectAttendance`'s composite indexes need an `institutionId`-stripped version once queries hit the nested path — steps 5–10 never actioned this, and it isn't "no data yet so it's harmless to defer" the way the rules/app-code phasing is: without these indexes, the already-converted nested-path queries in both attendance registers (§11 step 8's 12 files) would fail at runtime with a Firestore "the query requires an index" error the moment real traffic hits them post-cutover, regardless of how correct the query code itself is. Fixed now, ahead of the cutover, since new indexes are purely additive (creating one never breaks an existing query — Firestore just builds it in the background) — the same "safe to do anytime" property as additive rules blocks. Derived the exact field sets by grepping every converted call site rather than assuming §6.1's illustrative table was complete: `generalAttendance` needed one new index, `classId ASC · date ASC` (§6.1's table included a `session` field that, on inspection, no actual converted query filters or sorts by — dropped); `subjectAttendance` needed one new index, `subjectId ASC · classId ASC · sessionDate ASC`, matching §6.1's table exactly. Both added to `firestore.indexes.json` alongside (not replacing) the existing `institutionId`-inclusive versions, which stay live for the flat collections' queries until the deletion pass. `firebase-rules.md`'s "Composite index notes" section updated to document both new entries without altering the existing flat-index descriptions. Not yet deployed — like all indexes, deploys via `npm run firebase:deploy:indexes` (or `firebase:deploy` for rules+indexes together); safe to deploy anytime, but doesn't need to happen before the cutover specifically, since building an index takes effect immediately once the deploy completes and Firestore finishes indexing (typically fast at pilot scale) — recommended before or during the maintenance window, not after, so it isn't a race against the smoke test in cutover step 6.

This closes out every phase 5–12 build task plus this out-of-band index fix; the only work remaining on this branch is the cutover itself (§10.4).
