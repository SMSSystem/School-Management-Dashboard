...
3. At the actual cutover (§10.4 cutover step 4, right before/alongside the merge), apply every diff in §10.9 to `firestore.rules` in one pass, then deploy once.
...
Not in `firestore.rules`. Applied only at the cutover (§10.4 cutover step 4), immediately before that deploy.
...
12. **Documentation — done.** `firebase-rules.md`'s header already carries the CLI-is-authoritative convention change (§7.2) from earlier in this branch's history (the `af2de0c`/`bcd53e3` commits, before this session's step 5 work began) — confirmed still accurate, nothing further needed. No separate `docs/firebase/firestore_indexes` file exists in this repo; `firebase-rules.md`'s "Composite index notes" section already fills that role in already-retired form — it holds only the non-obvious "why" for two indexes (`generalAttendance`, `subjectAttendance`) and explicitly defers to `firestore.indexes.json` as the actual current inventory, so this bullet was effectively already satisfied by the existing structure — no edit needed. **Deleted the orphaned dead code flagged in step 6's writeup:** `src/scenes/(dashboard)/reports/index.tsx` and `src/lib/generateReport.ts` — confirmed via grep immediately before deleting that nothing else in `src/` imports either (the sibling `reports/builder/` directory, which *is* live, was left untouched), and confirmed via `tsc -b` afterward that removing them introduced zero new errors. `eslint`/`build` re-verified clean against the same known baseline. This closes out every phase 5–12 build task; the only work remaining on this branch is the cutover itself (§10.4).
