...
5. **Nest the low-traffic, no-cross-collection-dependency group.** `houses`, `departments`, `events`, `announcements`, `nonSchoolDays`, `academicYears` — simplest Pattern A collections (§5.2), nothing else reads from them via `get()`/`exists()` in rules. New `institutions/{id}/{collection}` rule blocks are additive-safe — deploy now, per §10.4 build step 1. App code changes: build now, merge deferred to the final cutover. Data copy deferred to the combined cutover (§10.4) — not migrated per phase.
