...
**Caveat, not yet resolved:** this convention change is written as taking effect once the first confirming deploy from `firestore.rules` actually succeeds (§11 step 1) — that confirming deploy had not been run as of this edit. Until it has, the Console's live state remains the practical source of truth in fact, even though the documentation now points at `firestore.rules`. Run the confirming deploy before treating this as fully in effect.
