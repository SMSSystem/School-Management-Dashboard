...
**Revised from the original estimate below** now that Phases 3-7 and 10-12 are implemented and finalized (Phase 4: 12 subjects, 8 institution-wide "core" + 4 class-scoped "elective", each class taking exactly 1 elective → **9 subjects/student**, not the ~8 originally assumed; Phase 10-12: attendance dates span the _full_ stored term range, not a "typical" ~63-school-day term) — see [§7.2](#72-phase-3-7-corrections-and-finalized-design-verified-against-actual-form-components) and [§7.3](#73-phase-10-12-corrections-and-finalized-design) for the full list of corrections this uncovered. `results`/`feedback_comments` (Phase 8/9) are still unbuilt — their rows below are projections updated for the 9-subject figure, not yet measured; every other changed row _is_ a measured, dry-run-verified count.

| Collection                                                                                 | Est. writes         | Driver                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------ | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `results` (Phase 8, not yet built)                                                         | ~56,000             | 1,250 students × **9** subjects/student × ~5 gradebook columns × 1 term (was ~50,000, assumed 8 subjects/student)                                                                       |
| `feedback_comments` (Phase 9, not yet built)                                               | ~11,250             | 1,250 students × **9** subjects × 1 term, deterministic upsert (was ~10,000)                                                                                                            |
...
| **Total**                                                                                  | **~99,000–103,000** | Up from ~96,000–100,000 — mainly `generalAttendance`'s ~2,850-write increase from the full-term-range correction ([§7.3](#73-phase-10-12-corrections-and-finalized-design))            |
...
```text
~101,000 total writes ÷ 10,000/day ≈ 10–11 daily runs to completion
```
