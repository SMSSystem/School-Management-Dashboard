...
>
> **PENDING DEPLOYMENT — Attendance Register Phase 1:** The `academicYears`, `nonSchoolDays`, and `generalAttendance` rule blocks below have been added to this document but not yet deployed to the Firebase Console. Add them to the deployed rules before using the attendance features. Additionally, create the following composite index in the Firebase Console: Collection `generalAttendance`, fields `institutionId ASC · classId ASC · date ASC · session ASC`.
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // ── Academic Years ─────────────────────────────────────────────────────
    // PENDING DEPLOYMENT — Attendance Register Phase 1
