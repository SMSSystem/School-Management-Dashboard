...
>
> **Composite index required:** The `generalAttendance` collection requires a composite index in the Firebase Console: Collection `generalAttendance`, fields `institutionId ASC · classId ASC · date ASC · session ASC`. Without this index, the live register query and overdue detection query will throw an error in production.
