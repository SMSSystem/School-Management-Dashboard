...
| Photo upload | Deferred | Avoids standing up a second public write surface (Firebase Storage rules + App Check enforcement on Storage) in the same effort as the new public Firestore write. The reference form's photo field is not replicated in v1. |
...
// The one deliberately public write in this entire app. Locked down as
// tightly as an unauthenticated create rule can be: every field the app
// code sets is validated, status must start at 'pending', and the
// conversion-tracking fields must be absent at create time (they're only
// ever set later, by an admin, via update).
match /institutions/{institutionId}/enrollmentRegistrations/{registrationId} {
  ...
  allow create: if institutionFieldMatchesPath(institutionId)
    ...
    && request.resource.data.possibleDuplicate is bool
    && request.resource.data.student.keys().hasAll(['lastName', 'firstName', 'requestedClass', 'dateOfBirth', 'gender'])
  ...
  allow update: if isAdminOrAbove()
    ...
    // Only the review/conversion-tracking fields may change after creation —
    // the submitted family data itself is immutable once submitted, the same
    // "audit trail" posture already used for disciplinaryActions edit rules.
...
- Row click opens a detail view: full submitted data (student + both guardian sections, whichever are present), plus action buttons:
  ...
  - **Reject** — sets `status: 'rejected'`. No further action possible on a rejected record from this UI (an admin who changes their mind deletes and asks the family to resubmit, rather than "un-rejecting" — keeps the state machine simple).
...
| Spark limit | Where it interacts with this feature | Assessment |
|---|---|---|
| Firestore reads/writes/deletes (50k/20k/20k per day) | One write per submission, a handful of reads per admin review session | Negligible at "handful of pilot institutions" scale — same conclusion as every other feature analyzed this way in this codebase |
| App Check | Free at any usage tier, not a Blaze-only product | No cost impact |
| No Cloud Functions | This entire feature is designed to avoid needing one (see §Design Decisions) | Confirmed zero new infrastructure cost |
| Abuse-driven quota exhaustion | The one real new risk: without App Check, a scripted flood of fake submissions could exhaust the daily write quota | Directly why App Check is in scope for this feature rather than deferred alongside photo upload/email |
...
2. Firestore rules: `registration_directory`, `enrollmentRegistrations`, the `student_parents` update-rule widening. Deploy and verify via the standard `git diff ... firestore.rules` safety check.
...
8. `/dashboard/registrations` review page: list, filters, detail view, status transitions.
...
- **Photo upload** — the reference form's "Student's Passport Sized Photograph" field. Requires public Firebase Storage rules and App Check enforcement on Storage in addition to Firestore; deliberately kept out of this pass (see §Design Decisions).
...
- **Un-rejecting a rejected submission** — v1's state machine treats `rejected` as terminal from the UI's perspective; an admin who changes their mind deletes and asks for a resubmission.
...
- **Rate-limiting beyond App Check** — App Check attests the request came from the real app; it does not by itself cap *how many* submissions one real visitor can send. Per-IP or per-session submission throttling is a possible future hardening step, not built here.
